...
async function main() {
  ...
  // Krok 6: Generowanie kluczy RSA
  console.log(`Generowanie kluczy RSA (${KEY_SIZE} bitów)...`);
  const { publicKey, privateKey } = await generateRSAKeys(KEY_SIZE);
  console.log('Klucze wygenerowane pomyślnie');

  // Krok 7: Szyfrowanie liczb
  ...
  // Krok 9: Konwersja liczb z powrotem na bloki tekstu
  ...
  // Krok 10: Połączenie bloków w celu utworzenia odszyfrowanej wiadomości
  const decryptedMessage = decryptedBlocks.join('');
  console.log(`Odszyfrowana wiadomość: ${decryptedMessage}`);

  // Krok 11: Weryfikacja poprawności odszyfrowania
  console.log('\nWeryfikacja:');
